...
class MLP:
    ...
    def feedforward(self, x):
        # Asegurarnos de que x siempre sea 2D (muestras, features)
        x = np.atleast_2d(x)  # convierte vectores 1D a (1, n_features)
        
        m = self._m(x)
        
        # Activación de entrada, con bias
...
def MLP_backprop_predict(X_train, y_train_encoded, X_test, alpha, lambda_, num_ite, verbose):
    """
    Función wrapper usada en pruebas:
